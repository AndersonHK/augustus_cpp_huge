...
- Loaded carts in cancel-order state are sent back to `src_storage_id` with no fallback if that id has already been cleared in [depot.c](../../src/figuretype/depot.c#L424).
- `set_destination()` is a no-op when `building_id` is 0 in [depot.c](../../src/figuretype/depot.c#L142).
- The clear-source and clear-destination UI handlers directly zero out the order ids in [window/building/depot.c](../../src/window/building/depot.c#L984) and [window/building/depot.c](../../src/window/building/depot.c#L994).
- There are some guarded reroute paths earlier in the depot state machine, but they do not cover the final `FIGURE_ACTION_244_DEPOT_CART_PUSHER_CANCEL_ORDER` fallback when source has already been cleared in [depot.c](../../src/figuretype/depot.c#L202) and [depot.c](../../src/figuretype/depot.c#L228).
...
- The new English depot status strings are present in [en.json](../../Mods/Augustus/Localization/en.json#L1411).
- The new English depot warnings are present in [en.json](../../Mods/Augustus/Localization/en.json#L1760).
- The new English depot/config keys are present in [en.json](../../Mods/Augustus/Localization/en.json#L2059).
- The French file ends before the corresponding config keys are added in [fr.json](../../Mods/Augustus/Localization/fr.json#L2046).

Why this is user-facing in this fork:
- Missing project keys fall back to the translation key name, not to English text, in [localization_runtime.cpp](../../src/translation/localization_runtime.cpp#L176).
- The fallback implementation explicitly stores the raw key name in [localization_support.cpp](../../src/translation/localization_support.cpp#L141).
...
Primary code evidence:
- The extra depot controls, including the Recall button, are only drawn inside the `if (data.advanced_mode)` block in [window/building/depot.c](../../src/window/building/depot.c#L561).
- In basic mode the mouse handler still enables the first 10 buttons, which includes `depot_order_buttons[9]` in [window/building/depot.c](../../src/window/building/depot.c#L592).
- The hidden button's callback is `depot_recall_all_cart_pushers()` in [window/building/depot.c](../../src/window/building/depot.c#L962).
