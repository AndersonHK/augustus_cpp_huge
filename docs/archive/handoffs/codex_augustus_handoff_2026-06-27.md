...
> Historical handoff snapshot. Paths and migration status below reflect 2026-06-27 and are not the current runtime contract.
...
- `Vespasian.sln`
- `Vespasian.vcxproj`
- Build only when useful for the current task. When building, use `Release|x64` with the project defaults such as `/O2`; MSBuild may need to be invoked directly from `<MSBuild path>`.
...
- HousingProfile XML owns resident class and shared residential requirement/tax/prosperity data.
