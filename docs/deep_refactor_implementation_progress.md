...
- [~] Review runtime logs for FigureType graphics fallback errors before the next native-figure graphics finishing slice. The latest available log (12:28) predates the current deployed artifact (18:02), so it cannot establish current fallbacks; capture and classify a fresh log after the next gameplay run.
