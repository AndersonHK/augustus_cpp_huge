...
Snapshot: 2026-08-31, branch `Vulkan-and-Refactors`
...
- Historical foundation baseline: `Rubble-and-Foundations` commit `844b35b68`.
- Current branch: `Vulkan-and-Refactors`.
...
- Current automated checkpoint: the configured mod stack is manifest-backed and sparse across all definition registries. The duplicate-layer cleanup removed 512 byte-identical non-Graphics definitions from Augustus and Vespasian. Clean Julius and Augustus extraction, Release builds, startup validation, dependency-specific Julius and Julius+Augustus save runs, and the complete 56-save Vespasian cohort all passed at 3,000 ticks and 1,000% speed with no unallowlisted warnings or errors. The deployed executable matched the Release artifact at that checkpoint.
- Manual gameplay validation remains required for composed-building rendering, figure alive/corpse presentation, rotated placement retention, access/perimeter behavior, repair of partially missing rubble compositions, and foundation terrain restoration across save/load. Do not treat headless parser coverage as a substitute for those interactive checks.
- Historical binary hashes and completed migration evidence belong in Git history and the archived implementation checklists; they are not maintained as current contracts here.
- Current validation priority: the user's upcoming manual visual and interaction pass, especially composed farms, figure presentation, foundation/composition repair, passage, demolition, and save/load behavior.
...
- [ ] Treat manual-test/deploy milestones as: terrain/water seam pixel checks, Vespasian scaled FigureType XML, deletion of strategy-obsolete compatibility branches, deletion of 16-soldier formation constants, and HDR scene target plus shader-side lighting/material policies.
...
- [x] Make `StartupParserTest.exe --game-root "<game install path>"` require `JuliusGraphicsExtractor` and `AugustusGraphicsExtractor` outputs before XML graphics validation.
...
Historical source plan: `docs/archive/implemented_plans/foundation_composition_housing_runtime_plan.md`
...
- [x] Extract graphics extraction into a self-contained DLL boundary. `graphics_extraction_abi.h` exposes versioned plain requests/results for Augustus extraction and climate-atlas bootstrap; runtime climate loading and extractor workflows dynamically load `GraphicsExtractor.dll`, copy scalar results, and unload, while concrete extractor classes remain private implementation details.
...
- [~] Extract save/load bridging into a self-contained module boundary. `VespasianLoadSave.dll` now owns bounded archive file input behind a versioned C ABI; saved-game and scenario callers receive caller-owned bytes and unload the module before parsing. The runtime no longer carries the duplicate FILE-based scenario decoder. Record translation, object hydration, export, and serialization still reside in the executable and remain the dominant boundary work.
- [x] Harden the first Load/Save ABI slice against missing files, oversized paths/archives, allocation failure, malformed query/fill results, future capability bits, reserved-field violations, exceptions crossing the C boundary, and module-handle leaks. `LoadSaveAbiTest` exercises archive reads, ownership, rejection, and unload.
...
- [x] Delete the duplicate dirty/rebuilt local-workforce house cache. Housing objects already self-publish into the authoritative `BuildingRuntimeList::Housing` index, so `RouteAccessSelector` filters that object list directly for populated labor sources while the workforce allocation table remains the only module-owned runtime state.
...
Gate: scaled Vespasian FigureType XML must wait until every `Renderer Scaling Seams` checklist item is complete, especially the source-pixels versus fixed-point-logical-size split.
...
- [~] Transition figure graphics to image group payload manager ownership with real file-path references instead of legacy group/image-id references. Active Vespasian FigureType definitions now use semantic payload paths with zero legacy source attributes. Consolidated auxiliary infantry and archer files demonstrate multi-entry animation ownership, while thin semantic aliases inherit a lower extracted group when its entries already form one coherent image group. Remaining work is the complete exceptional/enemy mapping ledger and removal of residual controller-owned legacy selection.
- [x] Add Vespasian 0.8-scale FigureType XML overrides using existing source art. Assetlist-level `logical_units_per_source_pixel="96"` now scales ordinary walkers, warriors, projectiles, animals, debris, carts, standards, corpses, and converted composite layers without changing source pixels; intentional UI and paper-map presentation stays unscaled.
...
Gate: this entire section blocks Vespasian scaled FigureType XML. The XML slice depends on source pixel dimensions being split from fixed-point logical image dimensions, and on the seam fixes being validated first.
...
- [x] Split source pixel dimensions from logical image dimensions. Managed runtime slices retain source raster width/height separately from a 120-unit-per-pixel fixed logical size. Authored image-group XML declares paired `logical_width`/`logical_height` values directly in those fixed-point units and must also declare paired source `width`/`height`; for example, a 46x50-pixel 1:1 image is `logical_width="5520" logical_height="6000"`. Entries without logical dimensions retain the legacy 1:1 source-sized output used by Julius and Augustus. Render submission scales the logical units through the existing camera zoom value. FigureType XML is forbidden from declaring logical dimensions or sprite offsets because those facts belong to the referenced graphics assetlist entry. The five shared 26,29-anchored walker families now carry that anchor on their 428 referenced animation, action, and death assetlists instead of their Augustus/Vespasian FigureTypes; the four portrait assetlists remain unanchored.
...
- [x] Unlock Vespasian scaled FigureType XML after the source/logical-size split and strict figure-owned payload path were complete. The enabled definitions pass the 4,608/4,608 terrain/water seam matrix, strict semantic materialization, and the 59-save 3,000-frame real-renderer gate with no unresolved or incomplete draw request.
...
- [x] Make barracks recruitment and overflow ejection consume `FormationType.recruit_capacity`; Vespasian explicitly authors 64 for its 8x8 century-scale infantry formations and 36 for its 6x6 cavalry turma.
- [x] Move legacy enemy and herd formation identity into Julius `FormationType` data with explicit spawn roles. Per-climate herd storage permits zero, one, or many active definitions, upper mods inherit/replace/suppress keys without engine branches, and every live formation requires a resolved immutable definition.
- [x] Move herd roaming, delay, reproduction, aggression, negative-desirability, and movement-sound behavior into the required resolved `FormationType`; remove sheep/zebra/wolf behavior switches from herd runtime.
- [ ] Close the spawn-definition validation gap: focused publication tests must cover unique enemy/herd figure ownership and per-climate indexing, and spawn slot composition must be homogeneous/full-grid until runtime supports true mixed spawning and stable mixed save identity.
...
- [x] Move distant-battle strength, trained bonus, and curse weighting from legion/non-legion branches into explicit `FormationType` combat policy. Missing strategic policy terminates at the object bottleneck rather than silently selecting a legacy default.
- [x] Collapse redundant formation accessors, movement-state helpers, and fort-binding wrappers; callers traverse resolved object relationships or invoke the owning `formation` method directly. Move `FormationLayoutDef` implementation out of the registry and into its matching class source.
