...
- `resource_set_mapping(resource_version)` installs the resource id mapping needed by old saves.
...
| 4 | `scenario_version` | `> SAVE_GAME_LAST_NO_SCENARIO_VERSION` | 4 | no | `buffer_write_i32(SCENARIO_CURRENT_VERSION)` | `save_version_to_scenario_version` | Embedded scenario format version |
| 5 | `image_grid` | `<= SAVE_GAME_LAST_STORED_IMAGE_IDS` | `image_grid` version size | yes | old saves only | `map_terrain_load_state` legacy image migration | Old stored map image ids |
| 6 | `edge_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_property_save_state` | `map_property_load_state` | Draw-tile / edge flags |
| 7 | `building_grid` | current | `GRID_SIZE_BUF_U16` or `GRID_SIZE_BUF_U32` | yes | `map_building_save_state` | `map_building_load_state` | Grid cell -> building id |
| 8 | `terrain_grid` | current | `GRID_SIZE_BUF_U16` or `GRID_SIZE_BUF_U32` | yes | `map_terrain_save_state` | `map_terrain_load_state` | Terrain bitfield grid |
| 9 | `aqueduct_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_aqueduct_save_state` | `map_aqueduct_load_state` | Aqueduct grid |
| 10 | `figure_grid` | current | `GRID_SIZE_BUF_U16` | yes | `map_figure_save_state` | `map_figure_load_state` | Grid cell -> first figure id |
| 11 | `bitfields_grid` | current | `GRID_SIZE_BUF_U8` or `GRID_SIZE_BUF_U16` | yes | `map_property_save_state` | `map_property_load_state` / `_u8` | Map property flags |
| 12 | `sprite_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_sprite_save_state` | `map_sprite_load_state` | Sprite grid |
| 13 | `random_grid` | current | `GRID_SIZE_BUF_U8` | no | `map_random_save_state` | `map_random_load_state` | Per-tile random bytes |
| 14 | `desirability_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_desirability_save_state` | `map_desirability_load_state` | Desirability grid |
| 15 | `elevation_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_elevation_save_state` | `map_elevation_load_state` | Elevation grid |
| 16 | `building_damage_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_building_save_state` | `map_building_load_state` | Building damage overlay grid |
| 17 | `aqueduct_backup_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_aqueduct_save_state` | `map_aqueduct_load_state` | Aqueduct backup grid |
| 18 | `sprite_backup_grid` | current | `GRID_SIZE_BUF_U8` | yes | `map_sprite_save_state` | `map_sprite_load_state` | Sprite backup grid |
| 19 | `figures` | current | dynamic after `SAVE_GAME_LAST_STATIC_VERSION` | yes | `figure_save_state` | `figure_load_state` | `array(figure)` records |
| 20 | `route_figures` | current | dynamic after `SAVE_GAME_LAST_STATIC_VERSION` | yes | `figure_route_save_state` | `figure_route_load_state` | Route id -> figure id table |
| 21 | `route_paths` | current | dynamic after `SAVE_GAME_LAST_STATIC_VERSION` | yes | `figure_route_save_state` | `figure_route_load_state` | Packed figure route directions |
| 22 | `formations` | current | dynamic after `SAVE_GAME_LAST_STATIC_VERSION` | yes | `formations_save_state` | `formations_load_state` | Military formations |
| 23 | `formation_totals` | current | 12 | no | `formations_save_state` | `formations_load_state` | Formation counters |
| 24 | `city_data` | current | resource-version-sized city block | yes | `city_data_save_state` | `city_data_load_state` | Main city globals |
| 25 | `city_faction_unknown` | `<= SAVE_GAME_LAST_UNKNOWN_UNUSED_CITY_DATA` | 2 | no | old saves only | skipped by old layout | Removed city faction payload |
...
| 29 | `buildings` | current | dynamic after `SAVE_GAME_LAST_STATIC_VERSION` | yes | `building_save_state` | `building_load_state` | `0xb6` building save heap; legacy `array(building)` records for old saves |
...
- Buildings: current `0xb6` saves write an owned heap object stream through `building_save_heap_write_current()`. The heap starts with `BSHP`, heap version, building count, and object count, then a linked sequence of objects with `{kind, version, object_id, payload_length, next_offset}` headers. `BuildingInstanceSaveObject` carries common per-instance state and save-local BuildingType ids; type-specific payload objects carry house, supplier, dock, depot, industry, rubble, roadblock, entertainment, monument, native-storage, native-production, and extension data. The old fixed building record serializer is now `building_state_legacy_save_to_buffer()` and is only for saves at or before `0xb5`.
- Building type table: `building_type_id_bridge_save_table_save_state()` writes dynamic format version `3`, entry count, each save id, text id, legacy enum hint, default graphics, and phased-construction data. Version `3` is mandatory for `0xb6`: XML/native phased construction is captured directly, and legacy monument construction is synthesized into the same payload before buildings are written. Version `1` and `2` readers remain old-save compatibility paths, but `0xb6` refuses to load them or fall back to legacy enum migration.
...
- `building_type_table` loads before `buildings`, because both the `0xb6` building heap and legacy building records store save-local building type ids.
...
Preview loading for savegame thumbnails uses the same piece reader, but it does not run the full live-game loader. It reads enough city/scenario/building/map data to render minimap info and uses `building_type_id_bridge_save_table_preview_load_state()` so save-local building ids can be interpreted without permanently replacing the runtime bridge.
