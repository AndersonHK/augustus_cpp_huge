...
int city_resource_get_available_empty_space_granaries(resource_type food)
{
    ...
    for (building *b = first_of_type("granary"); b; b = b->next_of_type) {
        ...
        available_storage += building_granary_maximum_receptible_amount(Building(b), food);
    }
...
{
    return static_cast<resource_trade_status>(city_data.resource.trade_status[resource]);
}
...
{
    for (int i = 0; i < RESOURCE_SLOT_COUNT; i++) {
        available.resources.items[i] = RESOURCE_NONE;
        available.foods.items[i] = RESOURCE_NONE;
        potential.resources.items[i] = RESOURCE_NONE;
        potential.foods.items[i] = RESOURCE_NONE;
    }
    available.resources.size = 0;
    available.foods.size = 0;
        ...
        if (map_has_road_access_granary(b->x, b->y, 0)) {
            ...
            if (pct_workers < 50) {
                ...
                for (int r = 0; r < RESOURCE_SLOT_COUNT; r++) {
                    ...
                    city_data.resource.granary_food_stored[r] += b->resources[r] * resource_units_per_load();
                }
                if (amount_stored > 4) {
...
static int house_consume_food(void)
{
    ...
    for (int i = 1; i < building_count(); i++) {
        ...
        for (resource_type r = (RESOURCE_NONE + 1); r < RESOURCE_SLOT_COUNT; r = static_cast<resource_type>(r + 1)) {
            if (b->resources[r] && resource_is_food(r)) {
                foodtypes_available++;
            }
        }
