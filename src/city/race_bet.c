...
void race_result_process(void)
{
    if (city_data.games.chosen_horse != NO_BET) {
        ...
        if (get_random_race_result() == city_data.games.chosen_horse) {
            ...
            city_warning_show(WARNING_BET_VICTORY, translation_for(TR_WARNING_BET_VICTORY));
        } else {
            ...
            city_warning_show(WARNING_BET_DEFEAT, translation_for(TR_WARNING_BET_DEFEAT));
        }
