...
static int export_event(scenario_event_t *event)
{
    ...
    unsigned int group_count = scenario_event_condition_group_count(event);
    for (unsigned int i = 0; i < group_count; i++) {
        scenario_condition_group_t *group = scenario_event_condition_group_get(event, i);
    ...
    unsigned int action_count = scenario_event_action_count(event);
    for (unsigned int i = 0; i < action_count; i++) {
        scenario_action_t *action = scenario_event_action_get(event, i);
