...
{
    return type == SCENARIO_BUILDING_ROAD ||
        type == SCENARIO_BUILDING_GARDENS ||
        type == SCENARIO_BUILDING_HIGHWAY ||
        type == SCENARIO_BUILDING_OVERGROWN_GARDENS ||
...
static int collapse_type_updates_tiles(int type)
{
    return collapse_type_is_terrain(type) ||
        type == SCENARIO_BUILDING_PLAZA ||
        type == SCENARIO_BUILDING_RUBBLE;
}

int scenario_action_type_change_allowed_buildings_execute(scenario_action_t *action)
{
    building_type building_id = static_cast<building_type>(action->parameter1);
...
{
    selected_rating rating = static_cast<selected_rating>(action->parameter1);
...
int scenario_action_type_change_resource_stockpiles_execute(scenario_action_t *action)
{
    ...
    storage_types storage_type = static_cast<storage_types>(action->parameter3);
...
int scenario_action_type_invasion_immediate_execute(scenario_action_t *action)
{
    ...
    scenario_invasion_start_from_action(static_cast<invasion_type_enum>(attack_type), size, invasion_point,
        static_cast<formation_attack_enum>(target_type), static_cast<enemy_type_t>(enemy_id));
...
int scenario_action_type_building_force_collapse_execute(scenario_action_t *action)
{
    ...
    if (collapse_type_updates_tiles(type) || destroy_all) {
...
int scenario_action_type_trade_add_new_resource_execute(scenario_action_t *action)
{
    ...
    resource_type resource = static_cast<resource_type>(action->parameter2);
...
{
    scenario_climate climate = static_cast<scenario_climate>(action->parameter1);
...
int scenario_action_type_change_production_rate_execute(scenario_action_t *action)
{
    ...
    if (set_to_value) {
        production_method_registry_set_production_per_month_for_resource(resource, rate);
    } else {
        production_method_registry_adjust_production_per_month_for_resource(resource, rate);
    }
