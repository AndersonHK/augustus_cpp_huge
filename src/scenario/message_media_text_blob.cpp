...
template <typename T>
static T *allocate_items(size_t count)
{
    return static_cast<T *>(std::malloc(sizeof(T) * count));
}

template <typename T>
static T *reallocate_items(T *items, size_t count)
{
    return static_cast<T *>(std::realloc(items, sizeof(T) * count));
}

static void resize_text_blob(size_t needed_space)
{
    if (!message_media_text_blob.max_size_text_blob) {
        message_media_text_blob.text_blob = allocate_items<uint8_t>(MESSAGE_MEDIA_TEXT_BLOB_INITIAL_SIZE);
        message_media_text_blob.max_size_text_blob = MESSAGE_MEDIA_TEXT_BLOB_INITIAL_SIZE;
    }
    size_t needed_max_size = message_media_text_blob.size + needed_space;
    if (message_media_text_blob.max_size_text_blob < needed_max_size) {
        size_t size = needed_max_size + MESSAGE_MEDIA_TEXT_BLOB_SIZE_INCREASE_OVERSHOOT;
        uint8_t *new_blob = reallocate_items(message_media_text_blob.text_blob, size);
        if (new_blob) {
...
static void resize_text_entries(size_t needed_entries)
{
    if (!message_media_text_blob.max_size_text_entries) {
        message_media_text_blob.text_entries = allocate_items<text_blob_string_t>(MESSAGE_MEDIA_TEXT_BLOB_INITIAL_ENTRIES);
        message_media_text_blob.max_size_text_entries = MESSAGE_MEDIA_TEXT_BLOB_INITIAL_ENTRIES;
    }
    size_t needed_max_count = message_media_text_blob.entry_count + needed_entries;
    if (message_media_text_blob.max_size_text_entries < needed_max_count) {
        size_t size = needed_max_count + MESSAGE_MEDIA_TEXT_BLOB_ENTRIES_INCREASE_OVERSHOOT;
        text_blob_string_t *new_blob = reallocate_items(message_media_text_blob.text_entries, size);
        if (new_blob) {
...
static size_t get_highest_entry_id(void)
{
...
void message_media_text_blob_clear(void)
{
    if (message_media_text_blob.size > 0) {
        std::memset(message_media_text_blob.text_blob, 0, message_media_text_blob.size);
        message_media_text_blob.size = 0;
        std::free(message_media_text_blob.text_blob);
        message_media_text_blob.text_blob = nullptr;
        message_media_text_blob.max_size_text_blob = 0;
        resize_text_entries(0);
    }

    if (message_media_text_blob.entry_count > 0) {
        std::memset(message_media_text_blob.text_entries, 0, message_media_text_blob.entry_count * sizeof(text_blob_string_t));
        message_media_text_blob.entry_count = 0;
        std::free(message_media_text_blob.text_entries);
        message_media_text_blob.text_entries = nullptr;
...
static text_blob_string_t *create_text_blob(size_t length)
{
    resize_text_entries(1);
    resize_text_blob(length);
    size_t offset = message_media_text_blob.size;
    size_t index = message_media_text_blob.entry_count;
...
const text_blob_string_t *message_media_text_blob_add_encoded(const char *text)
{
    ...
    encoding_from_utf8(text, output.data(), length);
    length = string_length(output.data()) + 1; // +1 to allow for end of string.
    text_blob_string_t *entry = create_text_blob(length);

    string_copy(output.data(), entry->text, length);
...
void message_media_text_blob_save_state(buffer *blob_buffer, buffer *meta_buffer)
{
    ...
    size_t array_size = message_media_text_blob.size;
    ...
    for (unsigned int i = 0; i < array_size; i++) {
        buffer_write_u32(meta_buffer, (unsigned int) message_media_text_blob.text_entries[i].id);
        buffer_write_u32(meta_buffer, (unsigned int) message_media_text_blob.text_entries[i].offset);
        buffer_write_u32(meta_buffer, (unsigned int) message_media_text_blob.text_entries[i].length);
    }
}

void message_media_text_blob_load_state(buffer *blob_buffer, buffer *meta_buffer)
{
    size_t size = buffer_load_dynamic_array(blob_buffer);
...
void message_media_text_blob_remove_unused(void)
{
    ...
    text_blob_string_t *new_text_entries = allocate_items<text_blob_string_t>(message_media_text_blob.max_size_text_entries);
    uint8_t *new_text_blob = allocate_items<uint8_t>(message_media_text_blob.max_size_text_blob);

    size_t index = 0;
    size_t new_size = 0;
    size_t new_offset = 0;
    for (size_t i = 0; i < message_media_text_blob.entry_count; i++) {
        if (message_media_text_blob.text_entries[i].in_use == 0) {
            continue;
        }
        size_t text_id = message_media_text_blob.text_entries[i].id;
        new_text_entries[index].id = text_id;
        new_text_entries[index].index = index;
        new_text_entries[index].in_use = 1;
        size_t length = message_media_text_blob.text_entries[i].length;
        size_t offset = message_media_text_blob.text_entries[i].offset;
