...
static std::deque<custom_message_t> custom_messages;
...
static custom_message_t new_entry(unsigned int position)
{
...
static custom_message_t *append_entry()
{
    custom_messages.push_back(new_entry(static_cast<unsigned int>(custom_messages.size())));
    return &custom_messages.back();
}
...
static custom_message_t *create_entry()
{
    for (size_t i = 1; i < custom_messages.size(); ++i) {
        if (!entry_in_use(custom_messages[i])) {
            custom_messages[i] = new_entry(static_cast<unsigned int>(i));
            return &custom_messages[i];
        }
    }
    return append_entry();
}
...
{
    if (!custom_messages.empty()) {
        for (custom_message_t &entry : custom_messages) {
...
{
    return message_id < 0 || static_cast<size_t>(message_id) >= custom_messages.size() ? nullptr : &custom_messages[message_id];
}

int custom_messages_get_id_by_uid(const uint8_t *message_uid)
{
    for (const custom_message_t &entry : custom_messages) {
        if (entry.linked_uid) {
...
{
    custom_message_t *entry = create_entry();
    if (!entry) {
        return 0;
    }
    entry->in_use = 1;
...
{
    return custom_messages.empty() ? 0 : static_cast<int>(custom_messages.size() - 1);
}

void custom_messages_save_state(buffer *buf)
{
    uint32_t array_size = static_cast<uint32_t>(custom_messages.size());
    uint32_t struct_size = (4 * sizeof(uint32_t));
    buffer_init_dynamic_array(buf, array_size, struct_size);

    for (const custom_message_t &message_entry : custom_messages) {
        const custom_message_t *entry = &message_entry;
...
void custom_messages_load_state(buffer *messages_buffer, buffer *media_buffer)
{
    ...
    custom_media_link_type link_type = CUSTOM_MEDIA_LINK_TYPE_UNDEFINED;
    int link_id = 0;
...
{
    for (custom_message_t &entry : custom_messages) {
        if (entry.linked_uid && entry.linked_uid->id == text_id) {
            entry.linked_uid = new_text_link;
            return 1;
        } else if (entry.title && entry.title->id == text_id) {
            entry.title = new_text_link;
            return 1;
        } else if (entry.subtitle && entry.subtitle->id == text_id) {
            entry.subtitle = new_text_link;
            return 1;
        } else if (entry.display_text && entry.display_text->id == text_id) {
