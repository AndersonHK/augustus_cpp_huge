...
{
    while (*a && *b) {
        int lhs = std::tolower(static_cast<unsigned char>(*a));
        int rhs = std::tolower(static_cast<unsigned char>(*b));
        if (lhs != rhs) {
            return lhs - rhs;
        }
        a++;
...
const dir_info *platform_file_manager_cache_get_dir_info(const char *dir)
{
    ...
    if (!info) {
        ...
        info->next = new dir_info{};
        info = info->next;
    }
    std::snprintf(info->name, FILE_NAME_MAX, "%s", dir);
    info->first_file = 0;
    info->next = 0;
    struct dirent *entry;
    ...
    if (*info->name) {
        dir_name_offset = std::snprintf(full_name, FILE_NAME_MAX, "%s/", info->name);
    }
    ...
    while ((entry = readdir(d))) {
        ...
        if (!file_item) {
            file_item = new file_info{};
            info->first_file = file_item;
        } else {
            file_item->next = new file_info{};
            file_item = file_item->next;
        }
        file_item->next = 0;

        // Copy name
        std::snprintf(file_item->name, FILE_NAME_MAX, "%s", name);
        std::snprintf(&full_name[dir_name_offset], FILE_NAME_MAX - dir_name_offset, "%s", name);

        // Copy extension
        file_item->extension = std::strrchr(file_item->name, '.');
        if (!file_item->extension) {
            file_item->extension = file_item->name + std::strlen(file_item->name);
        } else {
            file_item->extension++;
        }
...
void platform_file_manager_cache_update_file_info(const char *filename)
{
    ...
    for (current_file = base_dir_info->first_file; current_file; current_file = current_file->next) {
        if (std::strcmp(filename, current_file->name) == 0) {
            break;
        }
    }
    if (!current_file) {
        current_file = new file_info{};
        std::snprintf(current_file->name, FILE_NAME_MAX, "%s", filename);
    ...
    current_file->modified_time = static_cast<unsigned int>(std::time(nullptr));
}
