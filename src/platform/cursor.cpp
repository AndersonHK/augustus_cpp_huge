...
static SDL_Surface *generate_cursor_surface(const cursor *c)
{
    ...
    color_t *pixels = static_cast<color_t *>(cursor_surface->pixels);
...
void system_init_cursors(int scale_percentage)
{
    ...
    for (int i = 0; i < CURSOR_MAX; i++) {
        const cursor *list = input_cursor_data(static_cast<cursor_shape>(i), data.current_scale);
        if (data.surfaces[i]) {
...
{
    data.current_shape = static_cast<cursor_shape>(cursor_id);
    if (!platform_cursor_is_software()) {
