...
{
    int position = JOYSTICK_HAT_CENTERED;
    if (value & SDL_HAT_UP) {
    ...
    return static_cast<joystick_hat_position>(position);
}
...
static void set_default_controller_mapping(joystick_model *model, SDL_GameController *controller)
{
    for (int i = 0; i < NUM_CONTROLLER_MAPPINGS; i++) {
        ...
        SDL_GameControllerButtonBind binds[JOYSTICK_MAPPING_ELEMENTS_MAX] = {};
        int bind_failed = 0;
        for (int j = 0; j < JOYSTICK_MAPPING_ELEMENTS_MAX; j++) {
            int has_bind = 0;
            if (mapping->element[j].type == SDL_CONTROLLER_BINDTYPE_AXIS) {
                has_bind = 1;
                binds[j] = SDL_GameControllerGetBindForAxis(
                    controller,
                    static_cast<SDL_GameControllerAxis>(mapping->element[j].id));
            } else if (mapping->element[j].type == SDL_CONTROLLER_BINDTYPE_BUTTON) {
                has_bind = 1;
                binds[j] = SDL_GameControllerGetBindForButton(
                    controller,
                    static_cast<SDL_GameControllerButton>(mapping->element[j].id));
            }
