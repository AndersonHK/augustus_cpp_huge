...
static int xml_start_city(void)
{
    ...
    if (city_type < EMPIRE_CITY_DISTANT_ROMAN) {
        ...
        city_obj->city_type = static_cast<empire_city_type>(city_type);
    }
    ...
    city_obj->obj.future_trade_after_icon = static_cast<empire_city_icon_type>(future_trade_after_icon);
    ...
    if (city_obj->city_type == EMPIRE_CITY_TRADE || city_obj->city_type == EMPIRE_CITY_FUTURE_TRADE) {
        ...
        route_obj->obj.type = static_cast<empire_object_type>(xml_parser_get_attribute_enum("trade_route_type",
...
static void xml_end_distant_battle_path(void)
{
    empire_object_type obj_type = EMPIRE_OBJECT_ORNAMENT;
    int image_id = 0;
