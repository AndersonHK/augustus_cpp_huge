...
static void update_tool_bounds(void)
{
    if (data.current_tool > EMPIRE_TOOL_MAX) {
        data.current_tool = static_cast<empire_tool>(data.current_tool % (EMPIRE_TOOL_MAX + 1));
    }
}
...
void empire_editor_change_tool(int amount)
{
    if ((int)data.current_tool + amount < EMPIRE_TOOL_MIN) {
        data.current_tool = static_cast<empire_tool>(EMPIRE_TOOL_MAX - ((int)data.current_tool - amount) + 1);
    } else {
        data.current_tool = static_cast<empire_tool>((int)data.current_tool + amount);
    }
    update_tool_bounds();
}
...
int empire_editor_handle_placement(const mouse *m, const hotkeys *h, int outside_map)
{
    ...
    if (h->empire_tool) {
        data.current_tool = static_cast<empire_tool>(h->empire_tool - 1);
...
{    
    ...
    if (config_get(CONFIG_UI_EMPIRE_CONFIRM_DELETE)) {
        window_popup_dialog_show_confirmation(translation_for_key("TR_EMPIRE_DELETE_OBJECT"), 0, 0, deletion_confirmed);
    } else {
