...
static void select_asset(unsigned int index, int unused)
{
    ...
    if (img->id) {
        ...
        data.selected_asset_id = static_cast<char *>(malloc(id_length));
...
{
    scenario_climate climate = data.terrain == TERRAIN_NONE ?
...
static int update_asset_groups_list(void)
{
    ...
    data.xml_file_names = static_cast<const uint8_t **>(
        malloc(sizeof(uint8_t *) * data.xml_files->num_files));
    ...
    for (int i = 0; i < data.xml_files->num_files; i++) {
        ...
        int size = (int) strlen(original_file) + 1;
            uint8_t *file = static_cast<uint8_t *>(malloc(sizeof(uint8_t) * size));
...
static void create_selection_lists(void)
{
    ...
    uint8_t *text = static_cast<uint8_t *>(malloc(sizeof(uint8_t) * 5));
        if (!text) {
            continue;
        }
...
static void encode_asset_id(const char *id)
{
    ...
    if (size_needed > data.encoded_asset_id_size) {
        ...
        data.encoded_asset_id = static_cast<uint8_t *>(malloc(size_needed * sizeof(uint8_t)));
...
static void set_terrain(int type)
{
    if (data.terrain != type) {
        data.terrain = static_cast<terrain>(type);
        load_climate(0);
        window_invalidate();
    }
}
