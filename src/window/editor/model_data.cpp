...
static void handle_input(const mouse *m, const hotkeys *h)
{
    ...
    if (GenericButton::handle_mouse(*m_dialog, 0, 0, static_buttons, NUM_STATIC_BUTTONS, &data.static_buttons_focus_id)) {
        return;
    }
    ...
    if (GenericButton::handle_mouse(*m_dialog, x, y, data_buttons, NUM_DATA_BUTTONS, &data.data_buttons_focus_id)) {
        return;
    }
