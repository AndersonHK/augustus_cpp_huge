...
{
    return EMPIRE_TOOL_KEYS[tool];
}

static translation_key empire_city_icon_key(int icon)
{
    return EMPIRE_CITY_ICON_KEYS[icon];
}
...
static translation_key get_preview_translation_key(int tool_or_icon) {
    if (data.button_is_preview) {
        return empire_tool_key(tool_or_icon);
    } else {
        return empire_city_icon_key(tool_or_icon);
    }
}

static int draw_preview_image(int x, int y, int center, color_t color_mask, int draw_borders, int is_icon)
{
    int preview_image_group = get_preview_image_group(static_cast<empire_tool>(empire_editor_get_tool()));
        ...
        case EMPIRE_CITY_OURS:
        {
            ...
            for (int r = (RESOURCE_NONE + 1); r < RESOURCE_SLOT_COUNT; r++) {
                ...
                if (empire_object_city_sells_resource(city->empire_object_id, resource)) {
                    ...
                    draw_resource(resource, OUR_CITY, resource_x_offset, y_offset - 9, sell_buttons[r].highlighted);
                    sell_buttons[r] = {resource_x_offset, y_offset, 1, 0};
        ...
        case EMPIRE_CITY_FUTURE_TRADE:
        {
            ...
            for (int r = (RESOURCE_NONE + 1); r < RESOURCE_SLOT_COUNT; r++) {
                ...
                if (empire_object_city_sells_resource(city->empire_object_id, resource)) {
                    ...
                    width = draw_resource(resource, max_trade, resource_x_offset, y_offset - 9, sell_buttons[r].highlighted);
                    sell_buttons[r] = {resource_x_offset, y_offset, 1, 0};
            ...
            for (int r = (RESOURCE_NONE + 1); r < RESOURCE_SLOT_COUNT; r++) {
                ...
                if (empire_object_city_buys_resource(city->empire_object_id, resource)) {
                    ...
                    width = draw_resource(resource, max_trade, resource_x_offset, y_offset - 9, buy_buttons[r].highlighted);
                    buy_buttons[r] = {resource_x_offset, y_offset, 1, 0};
                    resource_x_offset += 32 + width;
                } else {
...
static void draw_object_info(void)
{
    ...
    if (obj->type == EMPIRE_OBJECT_ORNAMENT) {
        ...
        text_draw(translation_for(empire_ornament_key(ornament_id)),
            data.panel.x_min + 28, data.y_max - 125, FONT_NORMAL_GREEN,
    ...
    if (scenario.empire.id == SCENARIO_CUSTOM_EMPIRE) {
        ...
        if (!generic_buttons[7].parameter1) {
            ...
            empire_city_icon_type icon = empire_selected_object() ?
                ...
                EMPIRE_CITY_ICON_DEFAULT;
            text_draw_centered(translation_for(get_preview_translation_key(data.button_is_preview ? empire_editor_get_tool() : icon)),
...
static void handle_input(const mouse *m, const hotkeys *h)
{
    ...
    if (GenericButtonList(generic_buttons, scenario.empire.id == SCENARIO_CUSTOM_EMPIRE ? 8 : 1).handle_mouse(
        *m,
        data.panel.x_min + x_offset,
        data.y_max - 100,
    ...
    if (order_buttons[0].parameter2) {
        if (GenericButtonList(set_order_button, 1).handle_mouse(
            *m,
            data.panel.x_min,
            data.y_max - 160,
            NULL
    ...
    if (scenario.empire.id == SCENARIO_CUSTOM_EMPIRE && 
        GenericButtonList(top_buttons, 3).handle_mouse(
            *m,
            data.panel.x_max - 500,
            data.y_max - 133,
            &data.focus_top_button_id
        )) {
    ...
    if (scenario.empire.id == SCENARIO_CUSTOM_EMPIRE && 
        GenericButtonList(trade_city_buttons, 3).handle_mouse(
            *m,
            data.panel.x_max - 500,
            data.y_max - 93,
            &data.focus_city_button_id
        )) {
    ...
    if (scenario.empire.id == SCENARIO_CUSTOM_EMPIRE &&
        GenericButtonList(preview_button, 1).handle_mouse(
            *m,
            data.panel.x_max - 92,
            data.y_max - 100,
            &data.preview_button_focused
        )) {
        return;
    }
    if (scenario.empire.id == SCENARIO_CUSTOM_EMPIRE &&
        ...
        for (int r = RESOURCE_NONE; r < RESOURCE_SLOT_COUNT; r++) {
            ...
            if (m->x >= btn->x && m->x < btn->x + RESOURCE_ICON_WIDTH &&
                ...
                if (m->left.went_up && scenario.empire.id == SCENARIO_CUSTOM_EMPIRE && 
                    ...
                    data.selected_resource = resource;
                ...
                if (m->right.went_up && scenario.empire.id == SCENARIO_CUSTOM_EMPIRE) {
                    ...
                    if (i) {
                        empire_city_change_buying_of_resource(city, resource, 0);
                    } else {
                        empire_city_change_selling_of_resource(city, resource, 0);
                    }
                    window_request_refresh();
                    return;
                }
...
static void set_after_icon(int value)
{
    ...
    empire_object_get(selected_id - 1)->future_trade_after_icon = static_cast<empire_city_icon_type>(value);
}
...
static void button_after_icon(const generic_button *button)
{
    ...
    static int total_icons = EMPIRE_CITY_ICON_DEFAULT;
    for ( ; total_icons <= EMPIRE_CITY_ICON_TOWER; total_icons++) {
        icon_texts[total_icons] = translation_for(empire_city_icon_key(total_icons));
    }
    window_select_list_show_text((data.x_min + data.x_max) / 2 - 100, ((data.y_min + data.y_max) -
...
{
    empire_editor_set_tool(static_cast<empire_tool>(value));
}
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (btn->x + data.panel.x_max - 500 <= m->x && btn->x + data.panel.x_max - 500 + btn->width > m->x &&
            ...
            switch (i) {
                ...
                case 2:
                {
                    const uint8_t *raw_text = translation_for_key("TR_EMPIRE_TOOLTIP_AFTER_ICON");
                    const uint8_t *icon_text = translation_for(empire_city_icon_key(empire_object_get(
