...
static void create_resource_list_for_route(int route_id)
{
    ...
    for (int r = RESOURCE_MIN; r < RESOURCE_MAX; r++) {
        ...
        if (city_id != -1) {
            buys = empire_city_buys_resource(city_id, resource);
            sells = empire_city_sells_resource(city_id, resource);
        }
        if (buys) {
            route_resource_data.resource[r] = RESOURCE_BUYS;
            route_resource_data.traded_resources[route_resource_data.resource_list_size] = resource;
...
static void handle_input_resources(const mouse *m, const hotkeys *h)
{
    const mouse *m_dialog = mouse_in_dialog(m);
    if (scrollbar_handle_mouse(&scrollbar, m_dialog, 1) ||
        GenericButton::handle_mouse(*m_dialog, 0, 0, buttons, MAX_BUTTONS, &route_resource_data.focus_button_id)) {
        return;
    }
