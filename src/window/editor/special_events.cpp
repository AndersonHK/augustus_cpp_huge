...
static void draw_foreground(void)
{
    ...
    text_draw_number(scenario_editor_earthquake_year(), '+', " ", 340, 112, FONT_NORMAL_BLACK, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_BLACK)->line_height), 0);
    lang_text_draw_year(
        scenario_property_start_year() + scenario_editor_earthquake_year(),
        380, 112, FONT_NORMAL_BLACK, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_BLACK)->line_height));
...
static void handle_input(const mouse *m, const hotkeys *h)
{
    ...
    if (GenericButton::handle_mouse(*m_dialog, 0, 0, buttons, 15, &focus_button_id)) {
        return;
    }
    if (scenario.earthquake.severity == EARTHQUAKE_CUSTOM) {
