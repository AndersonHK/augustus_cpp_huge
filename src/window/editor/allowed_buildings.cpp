...
static unsigned int count_menu_items(build_menu_group menu)
{
    ...
    for (unsigned int i = 0; i < menu_items; i++) {
        ...
        if (submenu > SUBMENU_NONE) {
            if (submenu == menu) {
                continue;
            }
            count += count_menu_items(static_cast<build_menu_group>(submenu));
        } else {
            count++;
        }
    }
...
static void populate_menu_items(build_menu_group menu)
{
    ...
    for (unsigned int i = 0; i < menu_items; i++) {
        ...
        if (submenu > SUBMENU_NONE) {
            ...
            populate_menu_items(static_cast<build_menu_group>(submenu));
        } else {
            data.items[data.total_items].parent = menu;
...
static void populate_items(void)
{
    ...
    for (int group_index = BUILD_MENU_VACANT_HOUSE; group_index < BUILD_MENU_MAX; group_index++) {
        build_menu_group group = static_cast<build_menu_group>(group_index);
...
static void init(void (*on_select_callback)(int), int selected)
{
    ...
    data.selected_building = static_cast<building_type>(selected);
    populate_items();
...
static void draw_background(void)
{
    ...
    lang_text_draw_centered("main_strings.44.47", 26, 42, 608, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));
    ...
    if (current_menu->type == ITEM_TYPE_BUILDING) {
        ...
        if (building_menu_is_submenu(current_menu->menu)) {
            ...
            if (current_menu->building == BUILDING_NONE) {
                int width = lang_text_draw(current_string_key(68, current_menu->menu + 20), item->x + 4, item->y + 12, FONT_NORMAL_BLACK, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_BLACK)->line_height));
                ...
                draw_button(lang_get_string(current_string_key(68, current_menu->menu + 20)), current_menu->building,
