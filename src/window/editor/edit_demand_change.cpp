...
static void create_route_info(int route_id, const uint8_t *city_name)
{
    ...
    uint8_t *dst = static_cast<uint8_t *>(malloc(sizeof(uint8_t) * length));
    if (!dst) {
        return;
    }
...
static void init(int id)
{
    ...
    data.route_ids = static_cast<int *>(malloc(sizeof(int) * data.num_routes));
    data.route_names = static_cast<const uint8_t **>(malloc(sizeof(uint8_t *) * data.num_routes));
...
static void draw_background(void)
{
    ...
    text_draw_centered(resource_get_data(static_cast<resource_type>(data.demand_change.resource))->text,
...
{
    if (GenericButton::handle_mouse(*mouse_in_dialog(m), 0, 0, buttons, NUM_BUTTONS, &data.focus_button_id)) {
        return;
    }
