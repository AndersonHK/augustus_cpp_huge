...
static void init(int position)
{
    ...
    for (int i = 0; i < HOTKEY_MAX_ITEMS; i++) {
        ...
        mapping = hotkey_for_action(action, 1);
...
static void draw_background(void)
{
        ...
        if (widget->action == HOTKEY_HEADER) {
            ...
            if (mapping1->key) {
                ...
                text_draw_centered(keyname, HOTKEY_X_OFFSET_1 + 3, text_offset,
                    HOTKEY_BTN_WIDTH - 6, FONT_NORMAL_WHITE, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_WHITE)->line_height), 0);
    ...
    for (int i = 0; i < NUM_BOTTOM_BUTTONS; i++) {
        text_draw_centered(translation_for(bottom_button_texts[i]),
            ...
            bottom_buttons[i].width, FONT_NORMAL_BLACK, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_BLACK)->line_height), 0);
    }
...
static void handle_input(const mouse *m, const hotkeys *h)
{
    ...
    int handled = 0;
    handled |= GenericButtonList(hotkey_buttons, NUM_VISIBLE_OPTIONS * 2).handle_mouse(
        *m_dialog,
        0,
        0,
        &data.focus_button
                ...
                if (data.mappings[test_action][test_index].key == key
                    ...
                    if (!(test_action == action && test_index == index)) {
                        window_plain_message_dialog_show_with_extra(
                            ...
                            hotkey_action_name_for(static_cast<hotkey_action>(test_action)), 0);
                    }
                    break;
                }
            }
