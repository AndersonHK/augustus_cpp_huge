...
static void draw_foreground(void)
{
    ...
    if (resource_is_storable(data.resource)) {
        ...
        int seq_len = 0;
        storage_seq[seq_len++] = { LANG_FRAG_AMOUNT, 8, 10, warehouse_amount, 0, nullptr };
        storage_seq[seq_len++] = { LANG_FRAG_LABEL, 54, 15, 0, 0, nullptr };
        if (in_granaries > 0) {
...
static void button_toggle_trade(const generic_button *button)
{
    resource_trade_status status = static_cast<resource_trade_status>(button->parameter1);
