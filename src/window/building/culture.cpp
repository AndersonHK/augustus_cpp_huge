...
void window_building_draw_school(building_info_context *c)
{
    ...
    outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
    lang_text_draw_centered("main_strings.85.0", c->x_offset, c->y_offset + 10, 16 * c->width_blocks, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));

    if (!c->has_road_access) {
        window_building_draw_description(c, 69, 25);
    } else if (c->building->worker_count() <= 0) {
        window_building_draw_description(c, 85, 2);
    } else if (c->building->record()->upgrade_level) {
...
void window_building_draw_academy(building_info_context *c)
{
    ...
    outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
    lang_text_draw_centered("main_strings.86.0", c->x_offset, c->y_offset + 10, BLOCK_SIZE * c->width_blocks, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));
...
void window_building_draw_library(building_info_context *c)
{
    ...
    outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
    lang_text_draw_centered("main_strings.87.0", c->x_offset, c->y_offset + 10, 16 * c->width_blocks, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));
...
static void draw_temple_info(building_info_context *c, int image_offset)
{
    ...
    Building &site = *c->building;
    if (site.type && site.type->is_temple(GOD_CERES) && building_monument_gt_module_is_active(CERES_MODULE_2_DISTRIBUTE_FOOD)) {
    ...
    if (site.type && site.type->is_temple(GOD_VENUS) && building_monument_gt_module_is_active(VENUS_MODULE_1_DISTRIBUTE_WINE)) {
    ...
    int x_offset = 112;
    if (site.type && site.type->is_temple(GOD_MARS) && building_monument_gt_module_is_active(MARS_MODULE_1_MESS_HALL)) {
...
void window_building_draw_theater(building_info_context *c)
{
    ...
    Building &site = *c->building;
    auto *record = const_cast<building *>(site.record());
    if (!c->has_road_access) {
        window_building_draw_description(c, 69, 25);
    } else if (site.worker_count() <= 0) {
...
static void draw_temple(building_info_context *c, const char *sound_file, int group_id)
{
    ...
    if (c->building->monument_phase() <= 0) {
        ...
        window_building_play_sound(c, sound_file);
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
        lang_text_draw_centered(current_string_key(group_id, 0), c->x_offset, c->y_offset + 12, BLOCK_SIZE * c->width_blocks, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));
...
void window_building_draw_oracle(building_info_context *c)
{
    ...
    if (!c->building->is_unfinished_monument()) {
        ...
        outer_panel_draw(c->x_offset, c->y_offset, c->width_blocks, c->height_blocks);
        lang_text_draw_centered("main_strings.110.0", c->x_offset, c->y_offset + 12, 16 * c->width_blocks, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));
...
void window_building_draw_grand_temple_foreground(building_info_context *c, Temple &temple)
{
    ...
    if (religion && religion_has_legacy_god(*religion, GOD_MARS)) {
        window_building_draw_priority_buttons(c->x_offset + 50, c->y_offset + 135, temple);
        window_building_draw_delivery_buttons(c->x_offset + 392, c->y_offset + 40, temple);
    }
}
...
void window_building_draw_colosseum_background(building_info_context *c)
{
    ...
    if (is_arena || site.monument_phase() == MONUMENT_FINISHED) {
        ...
        if (is_arena) {
            ...
            lang_text_draw_centered("main_strings.74.0", c->x_offset, c->y_offset + 10, BLOCK_SIZE * c->width_blocks, FONT_LARGE_BLACK, screen_ui_to_pixel(font_definition_for(FONT_LARGE_BLACK)->line_height));
        }
        ...
        if (is_colosseum && !site.has_required_workers()) {
            ...
            if (active_games && active_games <= 3 && is_colosseum) {
                static const translation_key underway_games_text[] = {
                    {},
                ...
                lang_text_draw_amount(current_string_amount_key(8, 44, 2 * city_festival_games_remaining_duration()), 2 * city_festival_games_remaining_duration(), c->x_offset + width + 32, c->y_offset + 221, FONT_NORMAL_BROWN, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_BROWN)->line_height));
...
static void add_module(int selection)
{
    ...
    building_monument_add_module(const_cast<building *>(data.building->record()), data.module_choices[selection - 1]);
}
