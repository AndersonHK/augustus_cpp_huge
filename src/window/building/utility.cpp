...
static Building *rubble_building_from_context(building_info_context *c)
{
    ...
    return runtime_building_for_id(c->rubble_building_id);
}
...
void window_building_draw_roadblock_orders_foreground(building_info_context *c)
{
    ...
    for (unsigned int i = 0; i < size_of_orders_permission_buttons; i++) {
        ...
        int state = c->building ? Roadblock(*c->building).has_permission(static_cast<roadblock_permission>(i + 1)) : 0;
        if (state) {
...
void window_building_draw_burning_ruin(building_info_context *c)
{
    ...
    const building_type display_type = original_type ? original_type->type() :
        (rubble && rubble->type ? rubble->type->type() : BUILDING_NONE);
    ...
    text_draw(lang_get_building_type_string(display_type), c->x_offset + 32, c->y_offset + BLOCK_SIZE * c->height_blocks - 173, FONT_NORMAL_BLACK, screen_ui_to_pixel(font_definition_for(FONT_NORMAL_BLACK)->line_height), 0);
...
static void init_repair_building_button(building_info_context *c)
{
    ...
    repair_building_button->x = static_cast<short>(c->x_offset + (context_width - button_width) / 2);  // Center horizontally
    repair_building_button->y = static_cast<short>(c->y_offset + BLOCK_SIZE * c->height_blocks - 30);
...
void window_building_draw_well(building_info_context *c)
{
    ...
    int well_necessity = map_water_supply_is_building_unnecessary(c->building, 2);
    int text_id = 0;
...
void window_building_draw_latrines(building_info_context *c)
{
    ...
    int latrines_necessity = map_water_supply_is_building_unnecessary(c->building, 3);
    building *b = c->building ? const_cast<building *>(c->building->record()) : nullptr;
    if (b->num_workers <= 0) {
...
static void button_toggle_figure_state(const generic_button *button)
{
    ...
    Roadblock roadblock(*data.building);
