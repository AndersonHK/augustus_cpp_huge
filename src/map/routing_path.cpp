...
static int increase_direction_path_size(void)
{
    if (directions.total >= directions.size) {
        ...
        auto *new_path = static_cast<uint8_t *>(std::realloc(directions.path, new_size * sizeof(uint8_t)));
        if (!new_path) {
            return 0;
        }
...
{
    path->directions = static_cast<uint8_t *>(std::malloc(directions.total * sizeof(uint8_t)));
    if (!path->directions) {
        return 0;
    }
