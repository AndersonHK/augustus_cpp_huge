...
static void set_warning(warning_type *warning, warning_type type)
{
    if (warning) {
        *warning = type;
    }
...
static int is_edge(const map_tile *tile, warning_type *warning)
{
    int result = tile->x == 0 || tile->y == 0 || tile->x == map_grid_width() - 1 || tile->y == map_grid_height() - 1;
    if (!result) {
        set_warning(warning, WARNING_EDITOR_NEED_MAP_EDGE);
    }
    return result;
}
