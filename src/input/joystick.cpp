...
void joystick_update_element(int joystick_id, joystick_element element, int element_id, int value1, int value2)
{
    ...
    switch (element) {
        ...
        case JOYSTICK_ELEMENT_HAT:
            update_hat(&joystick->hat[element_id], static_cast<joystick_hat_position>(value1));
            break;
...
static int translate_hotkeys(void)
{
    ...
    for (int i = 0; i < MAX_HOTKEYS; ++i) {
        int value = get_joystick_input_for_action(static_cast<mapping_action>(HOTKEY_OFFSET + i), &data.joystick_hotkey[i]);
        handled |= value;
        if (value) {
