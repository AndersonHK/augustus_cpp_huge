...
void grid_box_draw(grid_box_type *grid_box)
{
    ...
    item.width = static_cast<int>(width) - grid_box->item_margin.horizontal;
    item.height = static_cast<int>(grid_box->item_height) - grid_box->item_margin.vertical;
    item.is_focused = 0;
    item.mouse.x = NO_POSITION;
    item.mouse.y = NO_POSITION;
