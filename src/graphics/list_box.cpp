...
void list_box_draw(list_box_type *list_box)
{
    ...
    if (list_box->draw_item) {
        list_box_item item = {};
        item.y = list_box->y + padding;
        item.width = static_cast<int>(width_blocks * BLOCK_SIZE) - padding * 2;
        item.height = static_cast<int>(list_box->item_height);
...
void list_box_handle_tooltip(const list_box_type *list_box, tooltip_context *c)
{
    ...
    list_box_item item = {};
    item.x = list_box->x + padding;
    item.y = list_box->y + padding + static_cast<int>(list_box->item_height);
    item.width = item_width;
    item.height = static_cast<int>(list_box->item_height);
    item.index = list_box->focus_button_id + list_box->scrollbar.scroll_position;
    item.position = list_box->focus_button_id;
    item.is_selected = item.index == list_box->selected_index;
    item.is_focused = 1;
