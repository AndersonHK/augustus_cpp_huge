...
void warning_draw(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (box_width < 460) {
            ...
            Image::from_id(Image::group(GROUP_CONTEXT_ICONS) + 15).draw(center - box_width / 2 + 2, top_offset + 2);
            Image::from_id(Image::group(GROUP_CONTEXT_ICONS) + 15).draw(center + box_width / 2 - 30, top_offset + 2);
        }
