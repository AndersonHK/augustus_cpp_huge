...
static int image_write_header(void)
{
    struct spng_ihdr ihdr = {
        .width = static_cast<uint32_t>(screenshot.width),
        .height = static_cast<uint32_t>(screenshot.height),
        .bit_depth = 8,
        .color_type = static_cast<uint8_t>(screenshot.alpha_channel ? SPNG_COLOR_TYPE_TRUECOLOR_ALPHA : SPNG_COLOR_TYPE_TRUECOLOR)
    };
...
static void show_saved_notice(const char *filename)
{
    ...
    const uint8_t *prefix = translation_for_key("TR_WARNING_SCREENSHOT_SAVED");
