...
void figure_play_die_sound(const Figure *f)
{
    ...
    if (is_soldier) {
        sound_effect_play(static_cast<sound_effect_type>(SOUND_EFFECT_SOLDIER_DIE + city_sound_update_die_soldier()));
    } else if (is_female_citizen) {
        sound_effect_play(SOUND_EFFECT_FEMALE_CITIZEN_DIE);
    } else if (is_citizen) {
        sound_effect_play(static_cast<sound_effect_type>(SOUND_EFFECT_CITIZEN_DIE + city_sound_update_die_citizen()));
    }
    if (f->is_enemy()) {
