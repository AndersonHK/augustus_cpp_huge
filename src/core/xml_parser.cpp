...
static void append_to_text(element_text *text_data, const char *text, int length)
{
    if (text_data->current_size + length > text_data->capacity) {
        ...
        char *new_text = static_cast<char *>(realloc(text_data->text, new_capacity));
        if (!new_text) {
...
static void handle_element_text(const sxmltok_t *token)
{
    ...
    if (data.current_element && data.current_element->on_text) {
        ...
        while (length) {
            const char *end = static_cast<const char *>(memchr(text, '\n', length));
...
static int expand_xml_token_array(void)
{
    ...
    sxmltok_t *expanded_tokens = static_cast<sxmltok_t *>(
        realloc(data.parser.tokens, sizeof(sxmltok_t) * expanded_size));
    if (!expanded_tokens) {
        return 0;
    }
...
int xml_parser_init(const xml_parser_element *elements, int total_elements, int stop_on_invalid_xml)
{
    ...
    data.elements = static_cast<xml_parser_element *>(malloc(elements_size));
    data.parents = static_cast<const xml_parser_element **>(malloc(sizeof(xml_parser_element *) * total_elements));
    data.texts = static_cast<element_text *>(malloc(sizeof(element_text) * total_elements));
...
static void increase_line_count(unsigned int position)
{
    ...
    for (int i = 0; cursor < end; i++, cursor++) {
        cursor = static_cast<const char *>(memchr(cursor, '\n', end - cursor));
        if (cursor == 0) {
...
int fit_buffer(int size)
{
    ...
    char *buffer = static_cast<char *>(realloc(data.buffer.data, size * sizeof(char)));
...
char *xml_parser_copy_attribute_string(const char *key)
{
    ...
    char *result = static_cast<char *>(malloc(buf_size));
    if (!result) {
        return 0;
    }
