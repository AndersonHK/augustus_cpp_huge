    ...
    while (*input && output < max_output) {
        if (*input < 0x80) {
            *output = *input;
            ...
            const japanese_entry key = {static_cast<uint16_t>(input[0] << 8 | (is_kata ? 0 : input[1]))};
            const japanese_entry *entry = static_cast<const japanese_entry *>(bsearch(&key, codepage_to_utf8,
    ...
    while (*input && output < max_output) {
        if ((*input & 0x80) == 0) {
            *output = *input;
            ...
            const japanese_entry *entry = static_cast<const japanese_entry *>(bsearch(&key, utf8_to_codepage,
...
int encoding_japanese_sjis_to_image_id(uint8_t first, uint8_t second)
{
    if (first < 0x80) {
        ...
        const japanese_image_entry key = {static_cast<uint16_t>((first << 8) | second)};
        const japanese_image_entry *entry = static_cast<const japanese_image_entry *>(bsearch(&key, image_entries,
