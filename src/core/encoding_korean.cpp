    ...
    while (*input && output < max_output) {
        if (*input < 0x80) {
            *output = *input;
            ...
            // multi-byte char
            const korean_entry key = {static_cast<uint16_t>(input[0] << 8 | input[1])};
            const korean_entry *entry = static_cast<const korean_entry *>(bsearch(&key, codepage_to_utf8,
    ...
    while (*input && output < max_output) {
        if ((*input & 0x80) == 0) {
            *output = *input;
            ...
            const korean_entry *entry = static_cast<const korean_entry *>(bsearch(&key, utf8_to_codepage,
