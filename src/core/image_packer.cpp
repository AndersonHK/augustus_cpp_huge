...
static int sort_rects(image_packer *packer)
{
    ...
    data->sorted_rects = static_cast<image_packer_rect **>(malloc(data->num_rects * sizeof(image_packer_rect *)));
...
int image_packer_init(image_packer *packer, unsigned int num_rectangles, unsigned int width, unsigned int height)
{
    ...
    packer->rects = static_cast<image_packer_rect *>(malloc(sizeof(image_packer_rect) * num_rectangles));
    if (!packer->rects) {
        return IMAGE_PACKER_ERROR_NO_MEMORY;
    }
    ...
    data->empty_areas.list = static_cast<empty_area *>(malloc(size * sizeof(empty_area)));
